...
- **Jade USB and QR (beta):** compatible desktop Chromium browsers communicate directly with Jade through its public CBOR-over-serial RPC protocol, verify a standard Native SegWit address on-device, and request only a message signature. A camera-equipped Jade Plus can instead remain air-gapped: the site scans Jade's animated public `crypto-account` xpub, derives the address locally, sends the native `signmessage ... ascii:...` request as animated `UR:BYTES`, then scans the Base64 signature response. The PIN is entered only on Jade; the browser can relay the firmware's encrypted blind-oracle request only to official Jade PIN-server HTTPS origins.
