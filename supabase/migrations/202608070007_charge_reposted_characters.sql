...
declare
  ...
  original_character_count integer := 0;
...
begin
  ...
  original_character_count := char_length(replace(coalesce(target_message.content, ''), E'\n', ''));
  quote_character_count := char_length(replace(coalesce(cleaned_quote, ''), E'\n', ''));
  billed_character_count := original_character_count + quote_character_count;
  repost_cost := billed_character_count * 0.00000001;

  if account.shells_balance < repost_cost then
    raise exception 'Solde insuffisant pour effectuer ce repost';
  end if;
  ...
  if repost_cost > 0 then
    ...
    set
      shells_balance = balances.shells_balance - repost_cost,
      shells_spent_total = coalesce(balances.shells_spent_total, 0) + repost_cost,
      last_sync = now()
